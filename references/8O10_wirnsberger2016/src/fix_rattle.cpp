...
{ 
  ...
  if (RATTLE_DEBUG) {
    ...
    if (npid == 0) {
      printf("RATTLE: Maximum overall relative position error ( (r_ij-d_ij)/d_ij ): %.10g\n", global_derr_max);
      printf("RATTLE: Maximum overall absolute velocity error (r_ij * v_ij): %.10g\n", global_verr_max);
    }
  }
}
