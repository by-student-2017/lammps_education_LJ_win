...
int main(int argc, char *argv[]){
  ...
  write=fopen("G_per_molecule_E-Esingle2_5_elec_equi","w");
  write2=fopen("InG_per_molecule_E-Esingle2_5_elec_equi","w");
  record=fopen("record_per_molecule_E-Esingle2_5_elec_equi","w");
  meanrecord=fopen("mean_replicate_etotal_single2_5_elec_equi.dat","w");
  recordfinal=fopen("Elattice_elec_equi.dat","w");
  /////////////////////////

  //assign space for evdwl, ecoul and elong and sum of these three
  ...
  for (i=0;i<Ni;i++){
    ...
    for (j=0;j<Nj;j++){  
	  ...
	  A[i][j]=D[i][j]+E[i][j]; //ecoul+evdwl-etail
      ...
      //initiate max[i] and min[i] as the first element 
      if (j==0){
	min[i]=A[i][j];
	max[i]=A[i][j];
      }
