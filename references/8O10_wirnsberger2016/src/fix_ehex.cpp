...
FixEHEX::FixEHEX(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  ...
  if (nevery <= 0) error->all(FLERR,"Illegal fix ehex command: integer value expected");
  ...
  while (iarg < narg) {
    ...
    // should constraints (shake/rattle) be applied again at the end of the timestep?

    else if (strcmp(arg[iarg], "constraints") == 0) {
      if (iarg+2 > narg) 
      ...
      if (strcmp(arg[iarg+1], "on") == 0)
        ...
        error->all(FLERR,"Illegal value for fix ehex option 'constraints'");
      iarg += 2;
    }

    // should a site only be rescaled if the entire molecule is contained within the region? 
      ...
      if (strcmp(arg[iarg+1], "on") == 0)
        ...
        error->all(FLERR,"Illegal value for fix ehex option 'com'");
      iarg += 2;
    }
  ...
  if (cluster && !constraints) 
    error->all(FLERR, "Illegal value for option 'cluster'. You can only use 'com on' together with 'constraints on' ");
  ...
  if (constraints) { 
    ...
    if (cnt_shake > 1)         
     ...
     fshake = ((FixShake*) modify->fix[id_shake]);
      ...
      error->all(FLERR, "Fix ehex was configured for constraints, but shake/rattle was not defined");
  }
}
...
void FixEHEX::rescale() {
  ...
  F     = heat_input * nevery * force->ftm2v;
