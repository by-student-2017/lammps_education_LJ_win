#!/bin/bash
#this scripts edit the input files and run simulations  

module load intel
rm -rf $node 
mkdir $node
cd $node
#copy all the input files and initial configuration from "IFile_shrink" here
#location2=$location/IFile_shrink --> read "submit_shrink_cavity.sh" 
\cp $location2/* ./

O=0      #labelling paramter
M=3500   #number of data samplings

#first-stage equilibration; 
\cp input_init_1.dat input_1.dat 

#second-stage equilibration (e.g. 1-4 ns)
#assign the equlibrilium lambda --> "submit_shrink_cavity.sh" for the meaning of lambda and other parameters
...
#"tail -7 log_2.lammps" --> the exact line depends on lammps version, not necessarily the first line of the last 7 line
...
rm -rf GE
######################################                                   
#assign a fixed gewald factor for pppm for data samplings
