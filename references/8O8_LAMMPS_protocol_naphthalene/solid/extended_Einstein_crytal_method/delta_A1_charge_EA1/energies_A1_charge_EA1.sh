...
mkdir $node
cd $node
#this is to copy all the input files from "IFile_A1_charge_EA1"
\cp $location2/* ./

N=400 #number of data samplings
M=240 #number of molecules in the crystal;
#We need M=240 to sample the intramolecular electrostatic energies one molecule by one molecule
#so that we can add them up to substract the sum from the total electrotatic energy, in order to obtain the total intermolecular electrostatic energy of the whole crystal at each sampling step

#"fixed" --> Orientational + Central Atoms/Springs
#"pair_coeff" --> atomic vdwl paramters
#read "submit_A1_charge_EA1.sh" for "$e11", "$e12" and "$e22".
#read input files in "IFile_A1_charge_EA1" to find out the different atom types in "set    group x charge".
################################  
...
\mv input_init.dat input_init_1.dat

head -26 input_init_1.dat > input_init.dat
echo "set             group C charge $chargecarbon" >> input_init.dat
echo "set             group Si charge $chargecarbon" >> input_init.dat
echo "set             group H charge $chargehydrogen" >> input_init.dat
echo "set             group S charge 0.000"  >> input_init.dat
echo "set             group Na charge $chargecarbon" >> input_init.dat
echo "set             group Ba charge 0.000" >> input_init.dat
echo "set             group O charge $chargecarbon" >> input_init.dat 
tail -79 input_init_1.dat >> input_init.dat
rm -rf input_init_1.dat
################################ 
...
\mv input.dat input_1.dat

head -26 input_1.dat > input.dat
echo "set             group C charge $chargecarbon" >> input.dat
echo "set             group Si charge $chargecarbon" >> input.dat
echo "set             group H charge $chargehydrogen" >> input.dat
echo "set             group S charge 0.000"  >> input.dat
echo "set             group Na charge $chargecarbon" >> input.dat
echo "set             group Ba charge 0.000" >> input.dat
echo "set             group O charge $chargecarbon" >> input.dat 
tail -79 input_1.dat >> input.dat
rm -rf input_1.dat
################################                                                                                                                                                                                                                           

#edit the input files accoriding to "k_TI.dat" for data samplings of intramolecular electrostatic energies
################################                                                                                                                                                                              
head -94 input_continue_single.dat > input_single2_1.dat
echo "fix            2 fixed spring/self $kmax" >> input_single2_1.dat
tail -17 input_continue_single.dat >> input_single2_1.dat
...
head -26 input_single2_3.dat > input_single2_4.dat
echo "set             group C charge $chargecarbon" >> input_single2_4.dat
echo "set             group Si charge $chargecarbon" >> input_single2_4.dat
echo "set             group H charge $chargehydrogen" >> input_single2_4.dat
echo "set             group S charge 0.000"  >> input_single2_4.dat
echo "set             group Na charge $chargecarbon" >> input_single2_4.dat
echo "set             group Ba charge 0.000" >> input_single2_4.dat
echo "set             group O charge $chargecarbon" >> input_single2_4.dat 
tail -79 input_single2_3.dat >> input_single2_4.dat
rm -rf input_single2_3.dat
################################                                                                                                                                                                                                                           
...
################################ 
#"tail -7 log_init_$O.lammps" --> the exact line depends on the lammps version, not necessarily the first line of the last 7 lines 
...
head -70 input_single2_4.dat > input_single2_4_1.dat
echo "kspace_modify  gewald $ge" input_single2_4.dat >> input_single2_4_1.dat
tail -41 input_single2_4.dat >> input_single2_4_1.dat 
\mv input_single2_4_1.dat input_single2_4.dat

head -70 input.dat > input_1.dat
echo "kspace_modify  gewald $ge" input.dat >> input_1.dat 
...
(for((i=1; i<$N+1; i++)); do
	...
	#sample N = 240 sample intramolecular electrostatic energies one molecule by one molecule
	####################################### 
        head -94 input_single2_4.dat > input_single2_5.dat
        echo "fix            2 fixed spring/self $kmax" >> input_single2_5.dat
        tail -17 input_single2_4.dat >> input_single2_5.dat

        head -25 input_single2_5.dat > input_single2.dat
        echo "group           single molecule != $j" >> input_single2.dat
        tail -86 input_single2_5.dat >> input_single2.dat
	rm -rf input_single2_5.dat input_single2_4.dat
